...
class ServerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($isParent == true) {
            ...
            $parent = ApiFunctions::returnParent($request->bearerToken());
            $servers = $parent->server()->whereIn('id', $accessible_servers)->paginate(5);
            return response()->json($servers);
        }
    }
    ...
    public function all(Request $request)
    {
        ...
        if ($isParent == true) {
            $user = ApiFunctions::returnUser($request->bearerToken());
            $servers = $user->server()->get();
            return response()->json($servers);
        } else {
            $user = ApiFunctions::returnUser($request->bearerToken());
            $accessible_servers = $user->servers;
            $parent = ApiFunctions::returnParent($request->bearerToken());
            $servers = $parent->server()->whereIn('id', $accessible_servers)->get();
            return response()->json($servers);
        }
    }
    ...
    public function information(Request $request, $id)
    {
        ...
        if ($isParent == true) {
            ...
            if (ApiFunctions::canAccessServer($user, $id)) {
                $parent = ApiFunctions::returnParent($request->bearerToken());

                $server = $parent->server()->where(['id' => $id])->firstOrFail();
                $api_instance = ApiFunctions::returnApiInstance($server);
                $type = ApiFunctions::returnType($api_instance);
