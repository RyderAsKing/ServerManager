...
class PterodactylFunctions
{
    public static function getPterodactylResources(Server $server, Api $api_instance)
    {
        ...
        if (isset($result['errors'])) {
            if ($result['errors'][0]['status'] == 403) {
                return ['status' => 419, 'error' => true, 'error_message' => 'The API key and password are incorrect'];
            } elseif ($result['errors'][0]['status'] == 404) {
                return ['status' => 419, 'error' => true, 'error_message' => 'The requested server was not found'];
            }
    ...
    public static function getPterodactyServers(Api $api_instance)
    {
        $host_ip = $api_instance->hostname;
        $key = $api_instance->api;
        ...
        curl_setopt($ch, CURLOPT_URL, $protocol . "://" . $host_ip . '/api/client/');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
